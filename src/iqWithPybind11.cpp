...
void process(const vector<string> &argv,
    ...
    */
    ...
    //--- start building
    ///  where hash_fn is a custom hash function for string_vector_view
...
// [[Rcpp::export]] -->[[pybind11::export]]
py::dict iq_filter(py:: str cmd) {

    // conver py::str to std::string
    std::string cmd_str = cmd.cast<std::string>();
    // const char *c = CHAR(STRING_ELT(cmd, 0));

    // get a C-style string
    const char *c = cmd_str.c_str();
    // const char *c = cmd_str[0];
    c = c + 0;  // This will point to the first character of cmd_str
...
/*
    ...
    convert vetor<string> to py::array_t and copy values

// */

    py::list p_index(protein_index->size());
    for (size_t i = 0; i < protein_index->size(); i++) {
        p_index[i] = protein_index->at(i);
    }

    py::list s_index(sample_index->size());
    for (size_t i = 0; i < sample_index->size(); i++) {
        s_index[i] = sample_index->at(i);
    }

    py::list i_index(ion_index->size());
    for (size_t i = 0; i < ion_index->size(); i++) {
        i_index[i] = ion_index->at(i);
    }

    py::list q(quant->size());
...
*/
    ...
    py::list ion_str(ions->size());
    for (size_t i = 0; i < ions->size(); i++) {
        // concatenate
        ion_str[i] = utils::concatenate(*(ions->at(i)));
    }

    py::list sample_str(samples->size());
    for (size_t i = 0; i < samples->size(); i++) {
        // concatenate
        sample_str[i] = samples->at(i);
    }

    py::list ann;
    for (size_t i = 0; i < annotation->size(); i++) {
        py::list list_value;
        for (size_t j = 0; j < (*annotation)[0]->size(); j++) {
            list_value.append((*(*annotation)[i])[j]);
        }
        ann.append(list_value);
    }

    py::list col_name(col_annotation->size());
    for (size_t i = 0; i < col_annotation->size(); i++) {
        // concatenate
        col_name[i] = col_annotation->at(i);
    }
...
/*
    ...
    set name col in ann for  col_name   convert with py::dict on keys = col_names

    ann ---> dimnames
*/
    py::dict dimnames;
    for (size_t i = 0; i < col_name.size(); i++){
       dimnames[col_name[i]] = ann[i];
    }
...
/*
    ...
    */
    ...
    py::dict quant_tab;
    quant_tab["protein_list"] = p_index;
    quant_tab["sample_list"] = s_index;
    quant_tab["id"] = i_index;
    quant_tab["quant"] = q;
...
*/
    ...
    py::dict vec;
    vec["quant_table"] = quant_tab;
    vec["protein"] = dimnames;
    vec["sample"] = sample_str;
    vec["ion"] = ion_str;
...
py::dict iq_MaxLFQ(py::dict list) {
    ...
    df.attr("columns") = py::tuple(py::iter(c_names));
    py::print("Check 2.\n");
    // Set row names
    // df.attr("index") = py::make_tuple(r_names.begin(), r_names.end());
    df.attr("index") = py::tuple(py::iter(r_names));
    py::print("Check 3.\n");
    // annotation
